...
anchored at the repo root. Non-SQLite URLs, `:memory:`, and already-absolute
SQLite URLs are returned unchanged.

Invariant: after this, every entry point (uvicorn from `backend/`, alembic
from `db/`, tests, CI) resolves the same URL to the same file on disk.
"""
...
def resolve_sqlite_url(url: str, repo_root: Path) -> str:
