...
def test_list_books_returns_effective_genre(client):
    ...
    client.patch(f"/items/{book_id}", json={"genre_override": "fantasy"})
    ...
    client.patch(f"/items/{book_id}", json={"genre_override": None})
    book = client.get("/items").json()[0]
...
def test_get_book_detail(client):
    book_id = _seed_one(client)
    res = client.get(f"/items/{book_id}")
    assert res.status_code == 200
    detail = res.json()
...
def test_book_404s(client):
    ...
    assert client.patch("/items/99999", json={}).status_code == 404
    assert client.post("/items/99999/skip").status_code == 404
    assert client.post("/items/99999/unskip").status_code == 404
...
def test_skip_hides_book_from_default_queue(client):
    ...
    assert client.post(f"/items/{book_id}/skip").status_code == 200

    # Default list omits skipped books
    assert client.get("/items").json() == []

    # But include_skipped=true surfaces them
    listed = client.get("/items?include_skipped=true").json()
    assert len(listed) == 1
...
def test_unskip_restores_visibility(client):
    book_id = _seed_one(client)
    client.post(f"/items/{book_id}/skip")
    assert client.get("/items").json() == []

    client.post(f"/items/{book_id}/unskip")
...
def test_patch_can_set_status_directly(client):
    ...
    book_id = _seed_one(client)
    assert client.patch(f"/items/{book_id}", json={"status": "published"}).status_code == 200
    assert client.get(f"/items/{book_id}").json()["status"] == "published"
...
def test_book_detail_exposes_dossier(client):
    book_id = _seed_one(client)
    assert client.get(f"/items/{book_id}").json()["dossier"] is None

    client.patch(f"/items/{book_id}", json={"dossier": _SAMPLE_DOSSIER})
    assert client.get(f"/items/{book_id}").json()["dossier"] == _SAMPLE_DOSSIER
...
def test_patch_dossier_replaces_wholesale(client):
    ...
    client.patch(f"/items/{book_id}", json={"dossier": new_dossier})

    assert client.get(f"/items/{book_id}").json()["dossier"] == new_dossier
...
def test_patch_dossier_null_clears(client):
    ...
    client.patch(f"/items/{book_id}", json={"dossier": None})
    assert client.get(f"/items/{book_id}").json()["dossier"] is None


def test_patch_dossier_rejects_non_object(client):
    book_id = _seed_one(client)
    res = client.patch(f"/items/{book_id}", json={"dossier": "not a dict"})
